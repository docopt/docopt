...
def test_pattern_flat():
    ...
    assert Required(Optional(OptionsShortcut()), Optional(Option("-a", None))).flat(
        OptionsShortcut
    ) == [OptionsShortcut()]
    ...
    assert parse_pattern("( -h | -v [ --file <f> ] )", options=o) == Required(
        Required(
            Either(
                Option("-h"),
                Required(
                    Option("-v", "--verbose"), Optional(Option("-f", "--file", 1, None))
                ),
            )
        )
    )
...
def test_basic_pattern_matching():
    # ( -a N [ -x Z ] )
    pattern = Required(
        Option("-a"), Argument("N"), Optional(Option("-x"), Argument("Z"))
    )
    # -a N
...
def test_pattern_either():
    ...
    assert transform(
        Optional(Option("-a"), Either(Option("-b"), Option("-c")))
    ) == Either(
